...
import { computeMorningReadiness, computeReadinessTrend, buildReadinessSeries } from "./morning/morningReadiness.js";
import { MORNING_DICT } from "./morning/morningI18n.js";
import MorningCheckinFlow from "./morning/MorningCheckinFlow.jsx";
import CmjDecisionStep from "./morning/CmjDecisionStep.jsx";
import MorningReadinessCard from "./morning/MorningReadinessCard.jsx";
import { buildMorningCheckinPayload } from "./morning/morningCheckinPersistence.js";
import { mapMorningCheckinToLegacyCheckin } from "./morning/legacyCheckinBridge.js";
...
// CMJ (Countermovement Jump) — module de biomarqueur neuromusculaire

// ------------------------------------------------------------

// Étape 1 (actuelle) : modèle de données + persistance uniquement.

// Aucune capture caméra ni calcul de baseline/tendance ici — ça arrive aux

// étapes suivantes (cmjEngine puis capture vision par ordinateur). Ce point

// d'entrée unique est celui que ces étapes futures appelleront pour

// persister un résultat de test, qu'il soit saisi manuellement (test) ou

// produit par la détection de saut réelle.

//

// `result` suit le modèle canonique défini dans l'architecture CMJ :

// { time, jumps: [{ heightCm, flightTimeMs, quality }], bestHeightCm,

//   avgHeightCm, testQuality, algorithmVersion, device, comments }

function saveCMJResult(athleteId, date, result) {

  if (!athleteId || !result) return;

  sheetsPost({ action: "saveCMJTest", athleteId, date, data: result });

}

function saveMorningCheckinResult(athleteId, date, data) {
  if (!athleteId || !data) return;
  sheetsPost({ action: "saveMorningCheckin", athleteId, date, data });
}
...
Object.keys(MORNING_DICT).forEach((l) => Object.assign(DICT[l], MORNING_DICT[l]));
...
function SessionCalendar({ sessions, setSessions, athleteId, readOnly, onOpenDebrief, notify, athleteName, t, lang }) {
  ...
  const weekLabel = `${fmtDate(dateKeyFromDate(weekStart), lang)} – ${fmtDate(dateKeyFromDate(weekEnd), lang)}`;

  const weekCells = useMemo(() => Array.from({ length: 7 }, (_, i) => addDaysToDate(weekStart, i)), [weekStart]);
  ...
  const isCurrentWeek = dateKeyFromDate(weekStart) === dateKeyFromDate(mondayOfWeek(TODAY));
  ...
  function selectDay(key) {
    ...
    setSelected(key); setEditingId(null);

    setWeekStart(mondayOfWeek(key));
  ...
  return (
    ...
    <div>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 12 }}>
        ...
        {["week", "month"].map(vm => (
          ...
          <button key={vm} onClick={() => setViewMode(vm)} style={{
            ...
            border: `1px solid ${viewMode === vm ? ACCENT + "88" : BORDER}`, background: viewMode === vm ? `${ACCENT}14` : "transparent",

            color: viewMode === vm ? ACCENT : MUTED,

          }}>{vm === "week" ? t("nav_calendar") + " — 7j" : t("nav_calendar") + " — 30j"}</button>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10, gap: 6 }}>
        ...
        <button onClick={() => viewMode === "week" ? setWeekStart(addDaysToDate(weekStart, -7)) : setCursor(new Date(year, month - 1, 1))} style={{ ...btnGhost, padding: 6 }}><ChevronLeft size={16} /></button>

        <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 600, fontSize: 13.5, textTransform: "capitalize", textAlign: "center", flex: 1 }}>

          {viewMode === "week" ? weekLabel : monthLabel}

        </span>

        <button onClick={() => viewMode === "week" ? setWeekStart(addDaysToDate(weekStart, 7)) : setCursor(new Date(year, month + 1, 1))} style={{ ...btnGhost, padding: 6 }}><ChevronRight size={16} /></button>

      </div>

      {!(viewMode === "week" ? isCurrentWeek : isCurrentMonth) && (

        <button onClick={goToday} style={{ ...btnGhost, borderColor: `${ACCENT}55`, color: ACCENT, marginBottom: 12, justifyContent: "center", width: "100%" }}>

          {t("nav_calendar")} · {fmtDate(TODAY, lang)}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 4, marginBottom: 16 }}>
        ...
        {viewMode === "week" ? weekCells.map((d, i) => {

          const key = dateKeyFromDate(d); const list = (sessions[athleteId] || {})[key] || []; const isSel = key === selected; const isToday = key === TODAY;

          return <button key={i} onClick={() => selectDay(key)} style={{ aspectRatio: "1", borderRadius: 9,
            ...
            <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: isToday ? ACCENT : MUTED, fontWeight: isToday ? 700 : 400 }}>{d.getDate()}</span>
...
function RecoveryDashboard({ checkins, debriefs, sessions, cycleCheckins, athlete, athleteId, cmjHistory, setCmjHistory, morningCheckinHistory, t, lang }) {
  ...
  if (!latest) {
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        <div style={{ color: MUTED2, fontSize: 13, textAlign: "center", padding: "20px 0 0" }}>{t("no_data")}</div>
        <button onClick={() => setShowCMJTest(true)} style={btnPrimary}>{t("cmj_run_test")}</button>
      </div>
    );
  }
  ...
  // ---- Morning Readiness (nouvelle architecture) ---------------------------
  const morningCheckinToday = (morningCheckinHistory?.[athleteId] || {})[TODAY] || null;
  const morningReadinessResult = computeMorningReadiness(morningCheckinToday, cmjReport?.status);
  // La tendance n'utilise que autonomic+recovery (pas le CMJ historique, qui
  // exigerait de recalculer une baseline glissante par jour — hors scope
  // de cette intégration, cf. limite documentée en Phase 7).
  const morningReadinessSeries = buildReadinessSeries(morningCheckinHistory?.[athleteId] || {}, TODAY, 7, () => null);
  const morningReadinessTrend = computeReadinessTrend(morningReadinessSeries);
  ...
  return (
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <MorningReadinessCard readiness={morningReadinessResult} trend={morningReadinessTrend} t={t} />
...
function NutritionModule({ athlete, setAthletes, sessions, debriefs, nutritionHistory, setNutritionHistory, t, lang }) {
  ...
  // Enregistre le plan nutritionnel du jour (objectifs + repas retenus) dans

  // l'historique du Google Sheet, avec un léger debounce. Contrairement à

  // syncAthleteProfile (préférences durables), ceci écrit une ligne par

  // date dans l'onglet "Nutrition" — l'historique des jours passés n'est

  // ainsi jamais écrasé par les changements du jour courant.
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      ...
      sheetsPost({
        ...
        action: "saveNutritionDay", athleteId, date: TODAY,

        data: { kcal: targets.kcal, carbs: targets.carbs, protein: targets.protein, fat: targets.fat, loadLevel, raceMode, meals: finalMeals },

      });

      setNutritionHistory(prev => ({ ...prev, [athleteId]: { ...(prev[athleteId] || {}), [TODAY]: { kcal: targets.kcal, carbs: targets.carbs, protein: targets.protein, fat: targets.fat, loadLevel, raceMode, meals: finalMeals } } }));

    }, 800);

    return () => clearTimeout(timeout);

  }, [targets.kcal, targets.carbs, targets.protein, targets.fat, loadLevel, raceMode, JSON.stringify(finalMeals)]);

  const historyDates = Object.keys(nutritionHistory?.[athleteId] || {}).filter(d => d !== TODAY).sort().reverse();
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ display: "flex", gap: 6 }}>
        ...
        {["plan", "history", "profile"].map(tb => (
          ...
          }}>{tb === "plan" ? t("meal_plan") : tb === "history" ? t("nutrition_history") : t("profile")}</button>
      ...
      {tab === "history" ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          {historyDates.length === 0 && <div style={{ color: MUTED2, fontSize: 12.5, textAlign: "center", padding: "16px 0" }}>{t("no_data")}</div>}

          {historyDates.map(d => {

            const entry = nutritionHistory[athleteId][d];

            const dayMeals = Object.entries(entry.meals || {}).filter(([, m]) => m);

            return (

              <Card key={d} label={fmtDate(d, lang)}>

                <div style={{ display: "flex", gap: 10, fontSize: 11.5, color: MUTED, marginBottom: dayMeals.length ? 8 : 0 }}>

                  <span>{entry.kcal} kcal</span><span>{entry.carbs}g {t("carbs").toLowerCase()}</span>

                  <span>{entry.protein}g {t("protein").toLowerCase()}</span><span>{entry.fat}g {t("fat").toLowerCase()}</span>
                ...
                {dayMeals.length > 0 && (
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                    ...
                    {dayMeals.map(([slot, m]) => (
                      ...
                      <div key={slot} style={{ fontSize: 11.5, color: MUTED2, display: "flex", justifyContent: "space-between" }}>

                        <span>{t(slot)} — {m.custom ? t("custom_meal") : m.name}</span>

                        <span style={{ fontFamily: "'JetBrains Mono', monospace", color: MUTED }}>{m.kcal} kcal</span>
...
function NutritionGate({ athlete, setAthletes, sessions, debriefs, nutritionHistory, setNutritionHistory, t, lang }) {
  ...
  return <NutritionModule athlete={athlete} setAthletes={setAthletes} sessions={sessions} debriefs={debriefs} nutritionHistory={nutritionHistory} setNutritionHistory={setNutritionHistory} t={t} lang={lang} />;
...
export { BG, SURFACE, BORDER, INK, MUTED, MUTED2, ACCENT, AMBER, RED, BLUE, TONE, inputStyle, btnPrimary, btnGhost, btnDanger, Card, MetricCard, Badge, uid, TODAY, saveCMJResult, saveMorningCheckinResult };
...
export default function BioSyncApp() {
  ...
  const [morningCheckinHistory, setMorningCheckinHistory] = useState({});
  ...
  async function loadAthleteData(athleteId) {
    ...
    if (data.nutrition) setNutritionHistory(prev => ({ ...prev, [athleteId]: { ...(prev[athleteId] || {}), ...data.nutrition } }));

    if (data.cmj) setCmjHistory(prev => ({ ...prev, [athleteId]: { ...(prev[athleteId] || {}), ...data.cmj } }));
    if (data.morningCheckins) setMorningCheckinHistory(prev => ({ ...prev, [athleteId]: { ...(prev[athleteId] || {}), ...data.morningCheckins } }));
  ...
  // Gestionnaire de complétion du nouveau Morning Check-in : construit le
  // payload plat pour la Sheet, sauvegarde, met à jour l'historique local,
  // ET alimente l'ancien format `checkins` via le pont de compatibilité
  // (legacyCheckinBridge.js) pour que le Tier 1 "Décision" du Dashboard
  // (Workload Engine existant) continue de recevoir des données réelles.
  function handleMorningCheckinComplete(athleteId, fullResult) {
    const payload = buildMorningCheckinPayload(fullResult);
    setMorningCheckinHistory(prev => ({ ...prev, [athleteId]: { ...(prev[athleteId] || {}), [TODAY]: payload } }));
    saveMorningCheckinResult(athleteId, TODAY, payload);

    const legacyCheckin = mapMorningCheckinToLegacyCheckin(fullResult);
    setCheckins(prev => ({ ...prev, [athleteId]: { ...(prev[athleteId] || {}), [TODAY]: legacyCheckin } }));
    sheetsPost({ action: "saveCheckin", athleteId, date: TODAY, data: legacyCheckin });

    notifyCoachEvent("checkin", athleteId, `Niveau ${fullResult.tier || "—"}`);
  }
    ...
    return (
      ...
      <Shell title={`${user.name}`} subtitle={t("space_coach")} tabs={tabs} tab={coachTab} setTab={setCoachTab} onLogout={logout}
        ...
        {coachTab === "dashboard" && <>
          ...
          {coachActiveAthlete && <RecoveryDashboard checkins={checkins} debriefs={debriefs} sessions={sessions} cycleCheckins={cycleCheckins} athlete={activeAthleteObj} athleteId={coachActiveAthlete} cmjHistory={cmjHistory} setCmjHistory={setCmjHistory} morningCheckinHistory={morningCheckinHistory} t={t} lang={lang} />}
        ...
        {coachTab === "nutrition" && <>
          ...
          <NutritionGate athlete={activeAthleteObj} setAthletes={setAthletes} sessions={sessions} debriefs={debriefs} nutritionHistory={nutritionHistory} setNutritionHistory={setNutritionHistory} t={t} lang={lang} />
    ...
    <Shell title={`${user.name.split(" ")[0]}`} subtitle={t("space_athlete")} tabs={tabs} tab={athleteTab} setTab={setAthleteTab} onLogout={logout}
      ...
      {athleteTab === "checkin" && (
        <MorningCheckinFlow
          ...
          cmjStep={CmjDecisionStep}
          cmjHistory={cmjHistory}
          setCmjHistory={setCmjHistory}
          sessionType={(sessions[user.athleteId]?.[TODAY] || []).some(s => s.discipline === "strength") ? "strength" : null}
          loadDeltaPercent={null}
          adaptiveReadinessScore={null}
          onComplete={(fullResult) => handleMorningCheckinComplete(user.athleteId, fullResult)}
          t={t}
          lang={lang}
        />
      )}
      ...
      {athleteTab === "dashboard" && <RecoveryDashboard checkins={checkins} debriefs={debriefs} sessions={sessions} cycleCheckins={cycleCheckins} athlete={me} athleteId={user.athleteId} cmjHistory={cmjHistory} setCmjHistory={setCmjHistory} morningCheckinHistory={morningCheckinHistory} t={t} lang={lang} />}
